...
import { ScenariosControl } from "../scenarios/ScenariosControl";

export function ProgramAndCost() {
  const { model } = useArena();
  const [drawerOpen, setDrawerOpen] = useState(false);
  // A focus request from an assumption chip: which control to scroll to, plus a
  // nonce so clicking the same chip twice re-triggers the scroll.
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-6">
      {/* Anchor */}
      <CostCommandBar onAdjustControl={handleAdjust} />
      ...
      <div className="mt-4 flex items-start">
        <ProgramDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} focus={focus} />
